...
describe('LKE ACL create tests', () => {
  ...
  it('creates an LKE cluster with ACL disabled', () => {
    ...
    cy.contains('Control Plane ACL (Access Control List)').should('be.visible');
    cy.contains(
   ...
   */
  it.only('creates an LKE cluster with ACL enabled', () => {
  ...
  /**
   * - Confirms validation error appears when a bad IP is entered
   * - Confirms API error appears when returned for acl field
   */
